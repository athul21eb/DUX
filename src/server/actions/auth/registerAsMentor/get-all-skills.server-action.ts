...
export const Get_All_Skills_For_Register_As_Mentor_Form_Server_Action = async():Promise<TSuccessResponse<Skill[]>|TErrorResponse>=>{
  ...
  try {
    ...
    return SuccessResponse('success fetched all skills',allSkills);
  } catch (error) {
  if (error instanceof ValidationError) {
    ...
    return ErrorResponse(
      error instanceof Error ? error.message : "failed to get all skills"
    );
  }
